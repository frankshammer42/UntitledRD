...
export default class Controls {
    ...
    addParticles(e) {
        ...
        //Get the current kick uv
        let u = e.clientX / w;
        let v = 1 - e.clientY / h;
        let tex = this.agents.outRenderTarget.texture;
        let arr = tex.image.data;
        let max = arr.length / 4;
        let ratio = w / h,
            a, r, id;
        for (let i = pid; i < pid + count; i++) {
            id = (i % max) * 4;    //if the pid over grows it just wraps around
    ...
    random() {
        ...
        this.renderer.copyTextureToTexture(new Vector2(0, 0), tex, tex);
        this.renderer.copyTextureToTexture(new Vector2(0, 0), tex, this.agents.outRenderTarget.texture);
    }
}
